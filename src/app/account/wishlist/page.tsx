...
/**
 * Profile รายการโปรด (wishlist tab) — the client explicitly asked
 * "ระบบนี้ทำได้ไหม" for saved products. Renders the wished products as
 * buyable cards; hearts here remove items via the same toggle API.
 */
...
export default function WishlistPage(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    fetch('/api/v1/wishlist/products', { credentials: 'include' })
    ...
    <div className="space-y-6">
      ...
      {products.length === 0 ? (
        <div className="clay-card flex flex-col items-center gap-4 p-10 text-center">
          ...
          <h2 className="text-lg font-bold text-fg">ยังไม่มีสินค้าในรายการโปรด</h2>
