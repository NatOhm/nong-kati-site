...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  ...
  pending_payment: { label: 'รอชำระเงิน', color: 'text-fg-brand bg-peach-100' },
  refunded: { label: 'คืนเงิน', color: 'text-coral-600 bg-coral-50' },
  expired: { label: 'หมดอายุ', color: 'text-fg-placeholder bg-surface' },
};
    ...
    <div className="space-y-6">
      ...
      {MOCK_ORDERS.length === 0 ? (
        <div className="rounded-md border border-line-subtle bg-white p-8 text-center">
          <p className="text-fg-placeholder">ยังไม่มีคำสั่งซื้อ</p>
        </div>
      ) : (
        <div className="space-y-3">
          {MOCK_ORDERS.map((order) => {
            const fallbackStatus = { label: 'ไม่ทราบ', color: 'text-fg-placeholder bg-surface' };
            const statusInfo = STATUS_LABELS[order.status] ?? fallbackStatus;
            return (
              <Link
                key={order.id}
                href={`/account/orders/${order.id}`}
                className="block rounded-md border border-line-subtle bg-white p-4 transition-colors hover:border-line-brand"
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-medium text-fg">{formatThb(order.total)}</p>
                    <span
