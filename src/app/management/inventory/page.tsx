      ...
      <div className="space-y-6">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-line-subtle bg-surface">
                <th className="px-4 py-3 text-left font-medium text-fg-muted">SKU</th>
                <th className="px-4 py-3 text-left font-medium text-fg-muted">สินค้า</th>
                <th className="px-4 py-3 text-right font-medium text-fg-muted">มูลค่า</th>
                <th className="px-4 py-3 text-center font-medium text-fg-muted">พร้อมใช้</th>
            ...
            <tbody>
              {allVariants.map((variant) => (
                <tr key={variant.id} className="border-b border-line-subtle hover:bg-white">
                  <td className="px-4 py-3 font-mono text-xs text-fg-muted">{variant.skuCode}</td>
                  <td className="px-4 py-3 text-fg">{variant.productName}</td>
                  <td className="px-4 py-3 text-right text-fg-secondary">฿{variant.faceValue}</td>
                  ...
                  <td className="px-4 py-3 text-center">
                    {variant.availableCount === 0 ? (
                      ...
                      <span className="text-xs text-fg-brand">ใกล้หมด</span>
                    ) : (
        ...
        {selectedVariant && (
          <div className="rounded-md border border-line-subtle bg-white p-6">
            ...
            <div className="mb-4 rounded-md border border-dashed border-line bg-surface p-8 text-center">
              <FileText size={32} className="mx-auto mb-2 text-fg-placeholder" />
              <p className="mb-2 text-sm text-fg-muted">
                ลากไฟล์ CSV มาที่นี่ หรือคลิกเพื่อเลือกไฟล์
              </p>
              <p className="mb-4 text-xs text-fg-placeholder">
            ...
            {uploadResult && (
              ...
              >
                <h3 className="mb-2 font-medium text-fg">ผลการอัปโหลด</h3>
                <div className="grid grid-cols-3 gap-4 text-sm">
                  <div>
                    <p className="text-fg-placeholder">ทั้งหมด</p>
                    <p className="font-bold text-fg">{uploadResult.totalRows}</p>
                  </div>
                  <div>
                    <p className="text-fg-placeholder">นำเข้าสำเร็จ</p>
                    <p className="font-bold text-jade-700">{uploadResult.importedCount}</p>
                  </div>
                  <div>
                    <p className="text-fg-placeholder">ปฏิเสธ</p>
                ...
                {uploadResult.rejectionDetails.length > 0 && (
                  <div className="mt-4">
                    <p className="mb-2 text-xs font-medium text-fg-muted">รายละเอียดการปฏิเสธ:</p>
                    <div className="max-h-40 space-y-1 overflow-y-auto">
                      {uploadResult.rejectionDetails.map((detail, idx) => (
                        <div
                          key={idx}
                          className="flex items-center gap-2 text-xs text-fg-placeholder"
                        >
            ...
            <button
              ...
              className="mt-4 text-sm text-fg-placeholder hover:text-fg"
            >
              ปิด
            </button>
          </div>
